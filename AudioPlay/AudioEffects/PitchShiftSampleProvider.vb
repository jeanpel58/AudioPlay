...
''' <summary>
''' Fournisseur d'échantillons qui applique un changement de pitch (tonalité)
''' Utilise la bibliothèque native SoundTouch (la même qu'Audacity) via P/Invoke
''' Architecture identique à TimeStretchSampleProvider (stable et éprouvée)
''' </summary>
Public Class PitchShiftSampleProvider
    ...
    Private tempReceiveBuffer As Single()  ' Buffer temporaire pour ReceiveSamples (évite corruption P/Invoke)
    ...
    Private _pitchSemiTones As Single = 0.0F

    ''' <summary>
    ''' Changement de pitch en demi-tons
    ''' 0 = normal, +12 = une octave plus haut, -12 = une octave plus bas
    ''' </summary>
    Public Property PitchSemiTones As Single
        Get
            Return _pitchSemiTones
        End Get
        Set(value As Single)
            If value >= -12.0F AndAlso value <= 12.0F Then
                _pitchSemiTones = value
                If soundTouch IsNot Nothing Then
                    soundTouch.SetPitchSemiTones(_pitchSemiTones)
                End If
            End If
        End Set
    End Property
    ...
    Public Sub New(source As ISampleProvider)
        ...
        outputBuffer = New Single(16383) {} ' Buffer de sortie plus grand pour le traitement
    ...
    Private Sub InitializeSoundTouch()
        ...
        Try
            ...
            soundTouch.SetSetting(SoundTouchInterop.SETTING_USE_QUICKSEEK, 0)  ' Désactiver quick seek (qualité)
        ...
        Catch ex As Exception
            System.Diagnostics.Debug.WriteLine($"Erreur initialisation SoundTouch (Pitch): {ex.Message}")
            soundTouch = Nothing
            isInitialized = True
        End Try
    End Sub
    ...
    Public Function Read(buffer() As Single, offset As Integer, count As Integer) As Integer Implements ISampleProvider.Read
        If Not Enabled OrElse Math.Abs(_pitchSemiTones) < 0.1F Then
            ' Passer le signal tel quel si désactivé ou pitch proche de 0
